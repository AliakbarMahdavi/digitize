...
import { dataPropvider } from './context/sortContextProvider';

const Sort = () => {

    const {Filter, state} = useContext(dataPropvider);
    ...
    return (
        <div className='hidden md:block md:col-span-9 xl:col-span-10 bg-white mx-4 h-[60px] rounded-md'>
            <div className='flex items-center h-full md:gap-x-4 lg:gap-x-8'>
                ...
                <button onClick={Filter} name="1" className={state === "1" ? 'h-full text-black md:text-[12px] lg:text-[16px] relative ' : 'h-full text-gray-400 md:text-[12px] lg:text-[16px]'}>
                    محبوب ترین محصول
                    {state === "1" ? <span className='w-2 h-2 bg-red-400 absolute top-3 rounded-full md:mr-0 lg:mr-2'></span> : null}
                    </button>    
                <button onClick={Filter} name="2" className={state === "2" ? 'h-full text-black md:text-[12px] lg:text-[16px] relative' : 'h-full text-gray-400 md:text-[12px] lg:text-[16px]'}>
                    پربازدید ترین محصول
                    {state === "2" ? <span className='w-2 h-2 bg-red-400 absolute top-3 rounded-full mr-2'></span> : null}
                    </button>    
                <button onClick={Filter} name="3" className={state === "3" ? 'h-full text-black md:text-[12px] lg:text-[16px] relative' : 'h-full text-gray-400 md:text-[12px] lg:text-[16px]'}>
                    گران ترین محصول
                    {state === "3" ? <span className='w-2 h-2 bg-red-400 absolute top-3 rounded-full mr-2'></span> : null}
                    </button>    
                <button onClick={Filter} name="4" className={state === "4" ? 'h-full text-black md:text-[12px] lg:text-[16px] relative' : 'h-full text-gray-400 md:text-[12px] lg:text-[16px]'}>
                    ارزان ترین محصول
